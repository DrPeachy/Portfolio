...
const FadeInScaleUpOnScroll = ({ children, start = "top 20%", end = "bottom top", duration = 0.2 }) => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      element,
      { scale: 0.8, opacity: 0 },
      { scale: 1, opacity: 1, duration: duration, ease: 'power3.out' }
    );
    ...
    const scrollTriggerInstance = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        gsap.to(element, {
          opacity: 1 - progress,
          scale: 1 - 0.2 * progress, // Shrinks and fades as you scroll
          ease: 'power2.out',
        });
      },
    });
